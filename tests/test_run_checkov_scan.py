...
import os
import pytest
from xebiams.terraform_mcp_server.impl.tools.run_checkov_scan import (
    _clean_output_text,
    _parse_checkov_json_output,
    run_checkov_scan_impl,
)
from xebiams.terraform_mcp_server.models.models import CheckovScanRequest
...
async def test_parse_checkov_json_output_valid():
    """Test parsing valid Checkov JSON output with Azure checks."""
    # Create a valid JSON output with Azure security checks
    json_output = {
        'results': {
            'failed_checks': [
                {
                    ...
                    'guideline': 'https://docs.bridgecrew.io/docs/azure-storage-encryption',
                    'severity': 'HIGH',
                },
                {
                    'check_id': 'CKV_AZURE_50',
                    'check_name': 'Ensure Virtual Machine is provisioned with managed disk',
                    'check_type': 'terraform',
                    'file_path': '/path/to/main.tf',
    ...
    assert vulnerabilities[0].id == 'CKV_AZURE_33'
    assert vulnerabilities[0].description == 'Ensure Storage account is using the latest version of TLS encryption'
    assert vulnerabilities[0].severity == 'HIGH'
    assert vulnerabilities[0].guideline == 'https://docs.bridgecrew.io/docs/azure-storage-encryption'

    assert vulnerabilities[1].id == 'CKV_AZURE_50'
    assert vulnerabilities[1].description == 'Ensure Virtual Machine is provisioned with managed disk'
...
async def test_run_checkov_scan_with_absolute_path(temp_terraform_dir):
    ...
    with patch('subprocess.run', return_value=mock_result):
        with patch(
            'xebiams.terraform_mcp_server.impl.tools.run_checkov_scan._ensure_checkov_installed',
            return_value=True,
        ):
            # Call the function
