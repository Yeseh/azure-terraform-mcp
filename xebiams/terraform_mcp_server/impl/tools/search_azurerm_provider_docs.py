...
"""Implementation of Azure RM provider documentation search tool."""
...
# Base URLs for Azure RM provider documentation
AZURERM_DOCS_BASE_URL = 'https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs'
GITHUB_RAW_BASE_URL = (
...
def resource_to_github_path(
    ...
    """Convert Azure RM resource type to GitHub documentation file path.

    Args:
        asset_name: The name of the asset to search (e.g., 'azurerm_storage_account')
    ...
    if not re.match(r'^[a-zA-Z0-9_-]+$', sanitized_name.replace('azurerm_', '')):
    ...
    # Remove the 'azurerm_' prefix if present
    if sanitized_name.startswith('azurerm_'):
        resource_name = sanitized_name[8:]
        logger.trace(f"[{correlation_id}] Removed 'azurerm_' prefix: {resource_name}")
    else:
        resource_name = sanitized_name
        logger.trace(f"[{correlation_id}] No 'azurerm_' prefix to remove: {resource_name}")
...
def fetch_github_documentation(
    ...
    Args:
        asset_name: The asset name (e.g., 'azurerm_storage_account')
...
def parse_markdown_documentation(
    ...
    try:
        ...
        if resource_match:
            ...
            # Fall back to getting text between the title and first level 2 heading
            ...
            if desc_match:
                ...
                description = f'Documentation for Azure {asset_name}'
        ...
        arg_ref_section_match = re.search(
            r'## Arguments?\s+Reference\n([\s\S]*?)(?=\n## |\Z)', content
        )
        if arg_ref_section_match:
            arg_section = arg_ref_section_match.group(1).strip()
            logger.debug(
                f'[{correlation_id}] Found Arguments Reference section ({len(arg_section)} chars)'
            )

            # Find arguments directly under the main Arguments Reference section
            ...
            if args_under_main_section_match:
                ...
                logger.debug(
                    f'[{correlation_id}] Found arguments directly under the Arguments Reference section ({len(args_under_main_section)} chars)'
                )
                ...
                logger.debug(
                    f'[{correlation_id}] Found {len(arg_list)} arguments directly under the Arguments Reference section'
                )
                ...
                for match in arg_list:
                    ...
                    if arg_name is not None and arg_desc is not None:
                        arguments.append(
                            {'name': arg_name, 'description': arg_desc}
                        )
                    logger.debug(
                        f"[{correlation_id}] Added argument '{arg_name}': '{arg_desc[:50] if arg_desc else 'No description found'}...' (truncated)"
                    )

            # Find all subheadings in the Arguments Reference section
            subheading_list = list(
                re.finditer(r'### (.*?)[\r\n]+(.*?)(?=###|\Z)', arg_section, re.DOTALL)
            )
            logger.debug(
                f'[{correlation_id}] Found {len(subheading_list)} subheadings in Arguments Reference section'
            )
            ...
            logger.debug(f'[{correlation_id}] No Arguments Reference section found')

        # Extract Attributes Reference section
        attributes = []
        attr_ref_match = re.search(r'## Attributes?\s+Reference\n([\s\S]*?)(?=\n## |\Z)', content)
        if attr_ref_match:
            attr_section = attr_ref_match.group(1).strip()
            logger.debug(
                f'[{correlation_id}] Found Attributes Reference section ({len(attr_section)} chars)'
            )
            ...
            logger.debug(
                f'[{correlation_id}] Found {len(attr_list)} attributes in Attributes Reference section'
            )
            ...
            logger.debug(f'[{correlation_id}] No Attributes Reference section found')

        # Extract Import section
        import_info = None
        import_match = re.search(r'## Import\n([\s\S]*?)(?=\n## |\Z)', content)
        if import_match:
            import_info = import_match.group(1).strip()
            logger.debug(f'[{correlation_id}] Found Import section ({len(import_info)} chars)')

        # Extract Timeouts section
        timeouts_info = {}
        timeouts_match = re.search(r'## Timeouts\n([\s\S]*?)(?=\n## |\Z)', content)
        if timeouts_match:
            timeouts_section = timeouts_match.group(1).strip()
            logger.debug(f'[{correlation_id}] Found Timeouts section ({len(timeouts_section)} chars)')
            
            # Parse timeout operations
            timeout_matches = re.finditer(
                r'\*\s+`([^`]+)`\s+-\s+(.*?)(?=\n\*\s+`|\n\n|\Z)',
                timeouts_section,
                re.DOTALL,
            )
            for match in timeout_matches:
                timeout_op = match.group(1).strip()
                timeout_desc = match.group(2).strip() if match.group(2) else 'No description available'
                timeouts_info[timeout_op] = timeout_desc
        ...
        return {
            ...
            'timeouts': timeouts_info if timeouts_info else None,
        }
        ...
        return {
            'title': f'Azure {asset_name}',
            'description': f'Documentation for Azure {asset_name} (Error parsing details: {str(e)})',
            'url': url,
...
async def search_azurerm_provider_docs_impl(
    ...
    Examples:
        - To get documentation for a storage account resource:
          search_azurerm_provider_docs_impl(asset_name='azurerm_storage_account')

        - To search only for data sources:
          search_azurerm_provider_docs_impl(asset_name='azurerm_storage_account', asset_type='data_source')

        - To search only for resources:
          search_azurerm_provider_docs_impl(asset_name='azurerm_virtual_machine', asset_type='resource')
    ...
    logger.info(f"[{correlation_id}] Starting Azure RM provider docs search for '{asset_name}'")
    ...
    try:
        ...
        logger.error(
            f'[{correlation_id}] Error searching Azure RM provider docs: {type(e).__name__}: {str(e)}'
        )
        # Don't log the full stack trace to avoid information disclosure
        ...
        return [
            TerraformAzureProviderDocsResult(
                ...
                asset_type=cast(Literal['both', 'resource', 'data_source'], asset_type),
                provider='azurerm',
                description='Failed to search Azure RM provider documentation. Please check your input and try again.',
                url=f'{AZURERM_DOCS_BASE_URL}/resources',
                example_usage=None,
                arguments=None,
                attributes=None,
                import_info=None,
