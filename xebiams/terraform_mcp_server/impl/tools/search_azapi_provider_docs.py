...
"""Implementation of Azure API provider documentation search tool."""
...
# Base URLs for Azure API provider documentation
AZAPI_DOCS_BASE_URL = 'https://registry.terraform.io/providers/Azure/azapi/latest/docs'
GITHUB_RAW_BASE_URL = (
    'https://raw.githubusercontent.com/Azure/terraform-provider-azapi/main/docs'
)
...
def resource_to_github_path(
    ...
    """Convert Azure API resource type to GitHub documentation file path.

    Args:
        asset_name: The name of the asset to search (e.g., 'azapi_resource')
    ...
    if not re.match(r'^[a-zA-Z0-9_-]+$', sanitized_name.replace('azapi_', '')):
    ...
    # Remove the 'azapi_' prefix if present
    if sanitized_name.startswith('azapi_'):
        resource_name = sanitized_name[6:]
        logger.trace(f"[{correlation_id}] Removed 'azapi_' prefix: {resource_name}")
    else:
        resource_name = sanitized_name
        logger.trace(f"[{correlation_id}] No 'azapi_' prefix to remove: {resource_name}")
...
def fetch_github_documentation(
    ...
    Args:
        asset_name: The asset name (e.g., 'azapi_resource')
...
async def search_azapi_provider_docs_impl(
    ...
    Examples:
        - To get documentation for an azapi resource:
          search_azapi_provider_docs_impl(asset_name='azapi_resource')

        - To search only for data sources:
          search_azapi_provider_docs_impl(asset_name='azapi_resource_list', asset_type='data_source')

        - To search only for resources:
          search_azapi_provider_docs_impl(asset_name='azapi_update_resource', asset_type='resource')
    ...
    logger.info(f"[{correlation_id}] Starting Azure API provider docs search for '{asset_name}'")
    ...
    try:
        ...
        logger.error(
            f'[{correlation_id}] Error searching Azure API provider docs: {type(e).__name__}: {str(e)}'
        )
        # Don't log the full stack trace to avoid information disclosure
        ...
        return [
            TerraformAzureProviderDocsResult(
                ...
                asset_type=cast(Literal['both', 'resource', 'data_source'], asset_type),
                provider='azapi',
                description='Failed to search Azure API provider documentation. Please check your input and try again.',
                url=f'{AZAPI_DOCS_BASE_URL}/resources',
                example_usage=None,
                arguments=None,
                attributes=None,
                import_info=None,
