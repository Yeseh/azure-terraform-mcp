...
async def search_user_provided_module_impl(
    ...
    try:
        ...
        # Try Azure-specific parsing first, then fall back to standard parsing
        from .utils import parse_azure_module_url, is_azure_module
        
        module_parts = parse_azure_module_url(request.module_url)
        if not module_parts:
