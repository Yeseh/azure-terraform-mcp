...
import time
import traceback
from xebiams.terraform_mcp_server.models import SubmoduleInfo, TerraformVariable
...
def parse_azure_module_url(module_url: str) -> Optional[Tuple[str, str, str]]:
    """Parse an Azure module URL to extract namespace, name, and provider.

    Supports both Terraform Registry and Azure marketplace URLs.

    Args:
        module_url: The module URL or identifier

    Returns:
        Tuple containing (namespace, name, provider) or None if invalid format
    """
    # Handle Azure marketplace URLs
    if 'azuremarketplace.microsoft.com' in module_url:
        logger.debug(f"Detected Azure marketplace URL: {module_url}")
        # Extract module info from Azure marketplace URL
        # Example: https://azuremarketplace.microsoft.com/en-us/marketplace/apps/publisher.offer
        marketplace_match = re.search(r'marketplace/apps/([^/]+)\.([^/?]+)', module_url)
        if marketplace_match:
            publisher = marketplace_match.group(1)
            offer = marketplace_match.group(2)
            # Convert to Terraform Registry format
            return publisher, offer, 'azurerm'
    
    # Handle Azure DevOps/GitHub URLs that might reference Azure modules
    if any(domain in module_url for domain in ['dev.azure.com', 'azure.com', 'github.com']):
        logger.debug(f"Detected Azure source URL: {module_url}")
        # Try to extract meaningful parts from the URL
        if 'github.com' in module_url:
            github_match = re.search(r'github\.com/([^/]+)/([^/]+)', module_url)
            if github_match:
                owner = github_match.group(1)
                repo = github_match.group(2)
                # Determine provider based on repository name patterns
                provider = 'azurerm'
                if 'azuread' in repo.lower():
                    provider = 'azuread'
                elif 'azapi' in repo.lower():
                    provider = 'azapi'
                return owner, repo, provider
    
    # Fall back to standard Terraform Registry URL parsing
    return parse_module_url(module_url)


def is_azure_module(namespace: str, name: str, description: str = '') -> bool:
    """Determine if a module is Azure-related based on various indicators.
    ...
    Returns:
        True if the module appears to be Azure-related
    """
    azure_indicators = [
        'azure', 'azurerm', 'azuread', 'azapi', 'az-', 'microsoft',
        'resource group', 'subscription', 'tenant', 'entra'
    ]
    
    search_text = f"{namespace} {name} {description}".lower()
    
    return any(indicator in search_text for indicator in azure_indicators)


def get_azure_module_conventions() -> Dict[str, Any]:
    """Get Azure-specific module conventions and patterns.

    Returns:
        Dictionary containing Azure module conventions
    """
    return {
        'providers': ['azurerm', 'azuread', 'azapi'],
        'common_namespaces': ['Azure', 'claranet', 'data-platform-hq', 'getindata', 'libre-devops'],
        'variable_patterns': {
            'naming': r'.*_(name|prefix|suffix)$',
        ...
        'output_patterns': {
            'id': r'.*_id$',
            'name': r'.*_name$',
            'endpoint': r'.*_(endpoint|url)$',
            'connection_string': r'.*_connection_string$',
        },
        'validation_patterns': {
            'location': r'contains\(\[.*\], var\.[^)]+\)',
            'sku': r'contains\(\[.*\], var\.[^)]+\)',
        }
    }
