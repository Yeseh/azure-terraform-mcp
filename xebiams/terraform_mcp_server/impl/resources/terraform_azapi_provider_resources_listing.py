...
"""Implementation for terraform_azapi_provider_resources_listing resource."""
...
async def terraform_azapi_provider_assets_listing_impl() -> str:
    """Generate a comprehensive listing of azapi provider resources and data sources.
    ...
    generate_azapi_provider_resources.py script.

    Returns:
        A markdown formatted string with categorized resources and data sources
    """
    logger.info('Loading azapi provider resources listing from static file')
    ...
    try:
        ...
        if STATIC_RESOURCES_PATH.exists():
            ...
            with open(STATIC_RESOURCES_PATH, 'r', encoding='utf-8') as f:
                content = f.read()
            logger.info('Successfully loaded azapi Provider asset list')
            return content
        else:
        ...
        logger.error(f'Error generating azapi provider assets listing: {e}')
        return f'# azapi Provider Assets Listing\n\nError generating listing: {str(e)}'
