...
"""Implementation for terraform_azurerm_provider_resources_listing resource."""
...
async def terraform_azurerm_provider_assets_listing_impl() -> str:
    """Generate a comprehensive listing of azurerm provider resources and data sources.
    ...
    generate_azurerm_provider_resources.py script.

    Returns:
        A markdown formatted string with categorized resources and data sources
    """
    logger.info('Loading azurerm provider resources listing from static file')
    ...
    try:
        ...
        if STATIC_RESOURCES_PATH.exists():
            ...
            with open(STATIC_RESOURCES_PATH, 'r', encoding='utf-8') as f:
                content = f.read()
            logger.info('Successfully loaded azurerm Provider asset list')
            return content
        else:
        ...
        logger.error(f'Error generating azurerm provider assets listing: {e}')
        return f'# azurerm Provider Assets Listing\n\nError generating listing: {str(e)}'
