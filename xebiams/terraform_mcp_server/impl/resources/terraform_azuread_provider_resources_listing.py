...
"""Implementation for terraform_azuread_provider_resources_listing resource."""
...
STATIC_RESOURCES_PATH = (
    Path(__file__).parent.parent.parent / 'static' / 'Azure AD_PROVIDER_RESOURCES.md'
)


async def terraform_azuread_provider_assets_listing_impl() -> str:
    """Generate a comprehensive listing of azuread provider resources and data sources.
    ...
    generate_azuread_provider_resources.py script.

    Returns:
        A markdown formatted string with categorized resources and data sources
    """
    logger.info('Loading azuread provider resources listing from static file')
    ...
    try:
        ...
        if STATIC_RESOURCES_PATH.exists():
            ...
            with open(STATIC_RESOURCES_PATH, 'r', encoding='utf-8') as f:
                content = f.read()
            logger.info('Successfully loaded azuread Provider asset list')
            return content
        else:
        ...
        logger.error(f'Error generating azuread provider assets listing: {e}')
        return f'# azuread Provider Assets Listing\n\nError generating listing: {str(e)}'
