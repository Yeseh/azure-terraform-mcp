...
"""Script to generate azapi provider resources markdown for the Terraform Expert MCP server.

This script scrapes the Terraform azapi provider documentation using Playwright
and generates a comprehensive markdown file listing all azapi service categories,
resources, and data sources.

The generated markdown is saved to the static directory for use by the MCP server.

Usage:
  python generate_azapi_provider_resources.py [--max-categories N] [--output PATH]

Options:
  --max-categories N    Limit to N categories (default: all)
  --output PATH         Output file path (default: terraform_mcp_server/static/azapi_PROVIDER_RESOURCES.md)
...
# Default output path
DEFAULT_OUTPUT_PATH = (
    Path(__file__).parent.parent.parent / 'static' / 'AZAPI_PROVIDER_RESOURCES.md'
)
# azapi provider URL
AZAPI_PROVIDER_URL = 'https://registry.terraform.io/providers/azure/azapi/latest/docs'
...
class ResourceItem(TypedDict):
    ...
    Attributes:
        name: The name/identifier of the resource (e.g. 'azapi_resource')
...
class ProviderResult(TypedDict):
    """Type definition for the result of fetching azapi provider data.

    Attributes:
        categories: Dictionary mapping azapi service category names to their resources and data sources
        version: azapi provider version string (e.g. "1.0.0")
    """
...
async def fetch_azapi_provider_page() -> ProviderResult:
    """Fetch the azapi provider documentation page using Playwright.

    This function uses a headless browser to render the JavaScript-driven
    Terraform Registry website and extract the azapi provider resources.
    ...
    Returns:
        ...
        - 'categories': Dictionary of azapi service categories with resources and data sources
        - 'version': azapi provider version string (e.g., "1.0.0")
    """
...
def get_fallback_resource_data() -> Dict[str, CategoryData]:
    ...
    Returns:
        A dictionary with pre-defined azapi resources and data sources
    """
    ...
    categories: Dict[str, CategoryData] = {
        'Resource': {
            'resources': [
                {
                    'name': 'azapi_resource',
                    'url': 'https://registry.terraform.io/providers/azure/azapi/latest/docs/resources/resource',
                    'type': 'resource',
                }
            ],
            'data_sources': [
                {
                    'name': 'azapi_resource',
                    'url': 'https://registry.terraform.io/providers/azure/azapi/latest/docs/data-sources/resource',
                    'type': 'data_source',
                }
            ],
        }
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Generate azapi provider resources markdown for the Terraform Expert MCP server.'
    )
    parser.add_argument(
        '--max-categories',
        type=int,
...
async def main():
    ...
    print('Generating azapi provider resources markdown...')
    ...
    try:
        # Fetch azapi provider data using the existing implementation
        result = await fetch_azapi_provider_page()
        ...
        # Generate markdown
        markdown = []
        markdown.append('# azapi Provider Resources Listing')
        markdown.append(f'\nazapi Provider Version: {provider_version}')
        markdown.append(f'\nLast updated: {datetime.now().strftime("%B %d, %Y %H:%M:%S")}')
        markdown.append(
            f'\nFound {total_resources} resources and {total_data_sources} data sources across {len(categories)} azapi service categories.\n'
        )
        ...
        markdown.append('---')
        markdown.append(
            '*This document was generated automatically by the azapi Provider Resources Generator script.*'
        )
        ...
        print(f'Error generating azapi provider resources: {str(e)}', file=sys.stderr)
        return 1
