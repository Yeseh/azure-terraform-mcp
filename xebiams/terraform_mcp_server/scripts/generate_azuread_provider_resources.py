...
"""Script to generate Azure AD provider resources markdown for the Terraform Expert MCP server.

This script scrapes the Terraform Azure AD provider documentation using Playwright
and generates a comprehensive markdown file listing all Azure AD service categories,
resources, and data sources.

The generated markdown is saved to the static directory for use by the MCP server.

Usage:
  python generate_azuread_provider_resources.py [--max-categories N] [--output PATH]

Options:
  --max-categories N    Limit to N categories (default: all)
  --output PATH         Output file path (default: terraform_mcp_server/static/Azure AD_PROVIDER_RESOURCES.md)
...
# Default output path
DEFAULT_OUTPUT_PATH = (
    Path(__file__).parent.parent.parent / 'static' / 'AZUREAD_PROVIDER_RESOURCES.md'
)
# Azure AD provider URL
AZUREAD_PROVIDER_URL = 'https://registry.terraform.io/providers/hashicorp/azuread/latest/docs'
...
class ResourceItem(TypedDict):
    ...
    Attributes:
        name: The name/identifier of the resource (e.g. 'azad_group')
...
class ProviderResult(TypedDict):
    """Type definition for the result of fetching Azure AD provider data.

    Attributes:
        categories: Dictionary mapping Azure AD service category names to their resources and data sources
        version: Azure AD provider version string (e.g. "5.91.0")
    """
...
async def fetch_azuread_provider_page() -> ProviderResult:
    """Fetch the Azure AD provider documentation page using Playwright.

    This function uses a headless browser to render the JavaScript-driven
    Terraform Registry website and extract the Azure AD provider resources.
    ...
    Returns:
        ...
        - 'categories': Dictionary of Azure AD service categories with resources and data sources
        - 'version': Azure AD provider version string (e.g., "5.91.0")
    """
...
def get_fallback_resource_data() -> Dict[str, CategoryData]:
    ...
    Returns:
        A dictionary with pre-defined Azure AD resources and data sources
    """
    ...
    categories: Dict[str, CategoryData] = {
        ...
        'Group': {
            'resources': [
                {
                    'name': 'azuread_group',
                    'url': 'https://registry.terraform.io/providers/hashicorp/azuread/latest/docs/resources/group',
                    'type': 'resource',
                }
            ],
            'data_sources': [
                {
                    'name': 'azuread_group',
                    'url': 'https://registry.terraform.io/providers/hashicorp/azuread/latest/docs/data-sources/group',
                    'type': 'data_source',
                }
            ],
        }
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Generate Azure AD provider resources markdown for the Terraform Expert MCP server.'
    )
    parser.add_argument(
        '--max-categories',
        type=int,
...
async def main():
    ...
    print('Generating Azure AD provider resources markdown...')
    ...
    try:
        # Fetch Azure AD provider data using the existing implementation
        result = await fetch_azuread_provider_page()
        ...
        # Generate markdown
        markdown = []
        markdown.append('# Azure AD Provider Resources Listing')
        markdown.append(f'\nAzure AD Provider Version: {provider_version}')
        markdown.append(f'\nLast updated: {datetime.now().strftime("%B %d, %Y %H:%M:%S")}')
        markdown.append(
            f'\nFound {total_resources} resources and {total_data_sources} data sources across {len(categories)} Azure AD service categories.\n'
        )
        ...
        print(f'Error generating Azure AD provider resources: {str(e)}', file=sys.stderr)
        return 1
