...
"""Script to generate azurerm provider resources markdown for the Terraform Expert MCP server.

This script scrapes the Terraform azurerm provider documentation using Playwright
and generates a comprehensive markdown file listing all azurerm service categories,
resources, and data sources.

The generated markdown is saved to the static directory for use by the MCP server.

Usage:
  python generate_azurerm_provider_resources.py [--max-categories N] [--output PATH]

Options:
  --max-categories N    Limit to N categories (default: all)
  --output PATH         Output file path (default: terraform_mcp_server/static/azurerm_PROVIDER_RESOURCES.md)
...
# Default output path
DEFAULT_OUTPUT_PATH = (
    Path(__file__).parent.parent.parent / 'static' / 'AZURERM_PROVIDER_RESOURCES.md'
)
# azurerm provider URL
AZURERM_PROVIDER_URL = 'https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs'
...
class ResourceItem(TypedDict):
    ...
    Attributes:
        name: The name/identifier of the resource (e.g. 'azurerm_resource')
...
class ProviderResult(TypedDict):
    """Type definition for the result of fetching azurerm provider data.

    Attributes:
        categories: Dictionary mapping azurerm service category names to their resources and data sources
        version: azurerm provider version string (e.g. "3.64.0")
    """
...
async def fetch_azurerm_provider_page() -> ProviderResult:
    """Fetch the azurerm provider documentation page using Playwright.

    This function uses a headless browser to render the JavaScript-driven
    Terraform Registry website and extract the azurerm provider resources.
    ...
    Returns:
        ...
        - 'categories': Dictionary of azurerm service categories with resources and data sources
        - 'version': azurerm provider version string (e.g., "3.64.0")
    """
    ...
    if not USE_PLAYWRIGHT or async_playwright is None:
        ...
        logger.info('Playwright is available and will be used to scrape the Azure AD provider docs')
        logger.info('Starting browser to extract Azure AD provider resources structure')
        ...
        try:
            async with async_playwright() as p:
                ...
                # Navigate to the Azure AD provider docs with reduced timeout
                logger.info(
                    f'Navigating to Terraform Azure AD provider documentation (timeout: {NAVIGATION_TIMEOUT}ms)'
                )
                try:
                    await page.goto(
                        AZURERM_PROVIDER_URL,
                ...
                # Extract Azure AD provider version
                provider_version = 'unknown'
                try:
                    # Try to extract version using the selector provided
                    logger.info('Attempting to extract Azure AD provider version')
                    ...
                    if version_element:
                        ...
                        if version_match:
                            ...
                            logger.info(f'Extracted Azure AD provider version: {provider_version}')
                        else:
                            ...
                            if provider_version:
                                ...
                                logger.info(
                                    f'Extracted Azure AD provider version via JavaScript: {provider_version}'
                                )
                    else:
                        ...
                        if provider_version:
                            ...
                            if version_match:
                                provider_version = version_match.group(1)
                                logger.info(
                                    f'Extracted Azure AD provider version via alternative selector: {provider_version}'
                                )
                except Exception as version_error:
                    logger.warning(f'Error extracting Azure AD provider version: {version_error}')
                ...
                with tempfile.NamedTemporaryFile(
                    prefix='terraform_azuread_debug_playwright_',
                    suffix='.html',
                    mode='w',
                    encoding='utf-8',
                    delete=False,
            ...
            logger.error(f'Error extracting Azure AD provider resources: {str(e)}')
...
def get_fallback_resource_data() -> Dict[str, CategoryData]:
    ...
    Returns:
        A dictionary with pre-defined azurerm resources and data sources
    """
            ...
            'resources': [
                {
                    ...
                    'url': 'https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/resource_group',
                    'type': 'resource',
                }
            ],
            'data_sources': [
                {
                    'name': 'azurerm_resource_group',
                    'url': 'https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/data-sources/resource_group',
                    'type': 'data_source',
                }
            ],
        }
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Generate azurerm provider resources markdown for the Terraform Expert MCP server.'
    )
    parser.add_argument(
        '--max-categories',
        type=int,
...
async def main():
    ...
    print('Generating azurerm provider resources markdown...')
    ...
    try:
        # Fetch azurerm provider data using the existing implementation
        result = await fetch_azurerm_provider_page()
        ...
        # Generate markdown
        markdown = []
        markdown.append('# azurerm Provider Resources Listing')
        markdown.append(f'\nazurerm Provider Version: {provider_version}')
        markdown.append(f'\nLast updated: {datetime.now().strftime("%B %d, %Y %H:%M:%S")}')
        markdown.append(
            f'\nFound {total_resources} resources and {total_data_sources} data sources across {len(categories)} azurerm service categories.\n'
        )
        ...
        markdown.append('---')
        markdown.append(
            '*This document was generated automatically by the azurerm Provider Resources Generator script.*'
        )
        ...
        print(f'Error generating azurerm provider resources: {str(e)}', file=sys.stderr)
        return 1
